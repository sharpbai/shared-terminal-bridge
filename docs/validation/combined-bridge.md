...
[Unix Socket Human Event 验证](unix-socket-events.md) 中由真人操作
验证通过。
...
```text
tests/host_tmux/test_combined_bridge_baseline.py
```
...
```bash
python3 -m unittest discover -s tests/host_tmux -p 'test_combined_bridge_baseline.py' -v
```
